...
impl NotesPanel {
    ...
    pub fn show(
        ...
        // Wrap entire content in a scroll area so nothing gets clipped
        egui::ScrollArea::vertical()
            .id_source("notes_content")
            .auto_shrink([false, false])
            .show(ui, |ui: &mut egui::Ui| {
                // Header with filter
                ui.horizontal(|ui: &mut egui::Ui| {
                    ui.label(RichText::new("Notes").size(16.0).color(theme::ACCENT));
                ...
                ui.horizontal(|ui: &mut egui::Ui| {
                    ...
                    if ui.button(RichText::new("New").color(theme::ACCENT)).clicked() {
                ...
                if visible.is_empty() {
                    ...
                    // Show up to 6 notes, then scroll
                    let note_list_h = (count as f32 * 24.0).min(150.0).max(40.0);

                    let mut note_to_select: Option<i64> = None;
                    egui::ScrollArea::vertical()
                        .id_source("note_list")
                        .max_height(note_list_h)
                        .auto_shrink([false, false])
                        .show(ui, |ui: &mut egui::Ui| {
